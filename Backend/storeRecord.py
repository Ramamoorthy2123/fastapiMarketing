...
SERVICE_ACCOUNT_FILE = 'Backend/NL_marketing_service_account.json'
...
# # Google Drive setup
# CLIENT_SECRETS_FILE = 'Backend/client_secrets.json'  # Your OAuth 2.0 client secrets JSON file
# SCOPES = ['https://www.googleapis.com/auth/drive.file']
# REDIRECT_URI = "http://localhost:8000/oauth2callback"  # Change this for production
...
#     image_upload: UploadFile = File(...),
#     visiting_card: Optional[UploadFile] = File(None),
#     access_token: str = Depends(oauth2callback)
# ):
#     try:
#         last_record = collection.find_one(sort=[("serial_number", -1)])
#         serial_number = last_record["serial_number"] + 1 if last_record else 1
...
#         # Use the access token to authenticate and create the Drive service
...
#         image_path = None
#         if image_upload:
#             image_path = await upload_to_drive(drive_service, image_upload, "images")
#             form_data["image_path"] = image_path

#         # Save the visiting card to Google Drive
#         visiting_card_path = None
#         if visiting_card:
#             visiting_card_path = await upload_to_drive(drive_service, visiting_card, "visiting_cards")
#             form_data["visiting_card_path"] = visiting_card_path
...
# async def upload_to_drive(drive_service, file: UploadFile, folder_name: str):
#     try:
#         # Create a folder in Google Drive if it doesn't exist
...
# def create_or_get_folder(drive_service, folder_name: str):
#     # Check if folder exists
#     query = f"mimeType='application/vnd.google-apps.folder' and name='{folder_name}'"
#     results = drive_service.files().list(q=query, fields="files(id)").execute()
#     items = results.get('files', [])
